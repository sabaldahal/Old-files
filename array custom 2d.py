...
count = len(n1)+1
#try 1 allData = [[0,0,0,0]*count] did not work        #declaring list of len(n1)+1 rows and 4 columns
allData = [[0]*4 for i in range(count)]


allData[0][0] = 'n1'                                        #adding the first row to the list
allData[0][1] = 'Theta1'
allData[0][2] = 'n2'
allData[0][3] = 'Theta2'

for o in range(len(n1)):                                    #adding values to the remaing rows of the list
...
for m in range(len(n1)+1):
    refractedfile.write('{0} \n'.format(allData[m]))                         #writing the list allData to the new file
